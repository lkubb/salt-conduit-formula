...
We will see a quick setup to configure the ``conduit`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``conduit`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``conduit`` formulas under ``/srv/salt/conduit/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/conduit/parameters/dns:domain/
    ln -s dns:domain /srv/salt/conduit/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/conduit/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/conduit/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``conduit`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``conduit`` formulas under ``/srv/salt/conduit/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/conduit/parameters/roles
...
We create a configuration for the role ``conduit/server`` in ``/srv/salt/conduit/parameters/roles/conduit/server.yaml``:
...
We create another configuration for the role ``conduit/client`` in ``/srv/salt/conduit/parameters/roles/conduit/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles conduit/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles conduit/client
...
.. code-block:: console
    ...
    mv /srv/salt/conduit/parameters/dns:domain/  '/srv/salt/conduit/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-conduit-as-json:
      test.nop:
        - name: {{ conduit | json }}
...
Here is an example based on `template-formula/conduit/config/file.sls`_:
    ...
    conduit-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='conduit-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ conduit.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``conduit`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ conduit['winner'] }}
...
.. _template-formula/conduit/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/conduit/config/file.sls
